...
class NeuralNetwork:
    def __init__(
        ...
        # Weights are initialized randomly (small values) so that
        # neurons starts with different parameters and can learn
        ...
        self.output_weighted_sum: np.ndarray = np.zeros((1, hidden_size))
    ...
    def forward_propagation(self, inputs: np.ndarray) -> np.ndarray:
        ...
        self.output_weighted_sum = (
            self.hidden_activation @ self.weights_hidden_output + self.bias_hidden
        )
